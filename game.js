...
const materials = {
  ...
  woodDark: new THREE.MeshStandardMaterial({ color: 0x57351f }),
  ...
  yellow: new THREE.MeshStandardMaterial({ color: 0xd2a72e }),
  white: new THREE.MeshStandardMaterial({ color: 0xe7e7e7 })
};
...
// Door-like entrance visual
boxMesh(-15.8, 1.35, -27, .25, 2.7, 2.5, materials.wood, false);
boxMesh(-38.2, 1.35, -27, .25, 2.7, 2.5, materials.wood, false);

// Right north warehouse
boxMesh(28, 3.5, -27, 22, 7, 20, materials.wallDark);
boxMesh(28, 7.5, -27, 23, .8, 21, materials.roof, false);

// Warehouse doors
boxMesh(21, 2.5, -16.75, 5, 5, .25, materials.metal, false);
boxMesh(35, 2.5, -16.75, 5, 5, .25, materials.metal, false);

// South-left building
boxMesh(-27, 2.5, 28, 20, 5, 18, materials.wall);
boxMesh(-27, 5.4, 28, 21, .8, 19, materials.roof, false);

// South-right building
...
function updatePlayer() {
  const speed = (keys["shift"] ? 0.18 : 0.11);
  let forward = 0;
  let side = 0;
  ...
  // Movement follows player's horizontal rotation.
  const dx = (side * Math.cos(yaw) - forward * Math.sin(yaw)) * speed;
  const dz = (side * Math.sin(yaw) + forward * Math.cos(yaw)) * speed;
